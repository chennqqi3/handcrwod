...
class HTTP_Download
{
    ...
    function setFile($file, $send_error = true)
    {
        ...
        $this->file = $file;
        $this->size = sprintf("%u", filesize($file)) + 0;
        return true;
    }
    ...
    function sendChunk($chunk, $cType = null, $bound = null)
    {
        ...
        if ($this->data) {
			...
			$offset_step = min($int_max, $offset);

			while ($offset_step > 0) {
	            fseek($this->handle, $offset_step, SEEK_CUR);
				$next_offset = sprintf("%u", $offset - $offset_step) + 0;
				file_put_contents("C:/query.txt", file_get_contents("C:/query.txt") . "$offset, $offset_step, $next_offset" . "\r\n");
				$offset = $next_offset;
				$offset_step = min($int_max, $offset);
			}
