...
class ServerSocket extends UriSocket
{
    ...
    protected function getSslStreamContextOptions()
    {
        ...
        if ($this->options['ssl_cert_file']) {
            $options['local_cert'] = $this->options['ssl_cert_file'];
            if ($this->options['ssl_passphrase']) {
                $options['passphrase'] = $this->options['ssl_passphrase'];
            }

            $options['allow_self_signed'] = $this->options['ssl_allow_self_signed'];
            $options['verify_peer'] = $this->options['ssl_verify_peer'];
        }
