...
class Connection extends Configurable
{
    ...
    public function handshake($data)
    {
        try {
            ...
            $this->params = array_slice(explode('/', $path), 2);
            ...
            if (method_exists($this->application, 'onConnecting')) {
                $this->application->onConnecting($this);
            }
    ...
    {
        if ($value == "@no_val@") {
            if (!is_array($this->sessions) || !array_key_exists($name, $this->sessions))
                return null;
