...
{   
			...
			foreach($changed_sockets as $socket)
			{
				if($socket == $this->master)
				{
					if(($resource = @stream_socket_accept($this->master)) === false)
					{
						...
						if ($errno != 0)
							$this->log('Socket error: ' . socket_strerror($errno));
						continue;
					}
					else
					{
						...
						{
							$client->log('Client Limit Reached!');
							$client->onDisconnect();
						...
						{
							$client->log('Connection/Ip limit for ip ' . $client->getClientIp() . ' was reached!');
							$client->onDisconnect();
							if($this->getApplication('status') !== false)
							{
	...
	{		
        $client->log('removeClientOnError', 'info');
		// remove reference in clients app:
		if($client->getClientApplication() !== false)
		{
        	$client->log('call onDisconnect', 'info');
