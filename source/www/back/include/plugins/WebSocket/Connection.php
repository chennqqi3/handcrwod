...
class Connection
{
    ...
    {	
		...
		$path = $matches[1];
		$path_params = preg_split("/\//", substr($path, 1));
		$this->application = $this->server->getApplication($path_params[0]);
        if(!$this->application)
		{
            $this->log('Invalid application: ' . $path_params[0]);
		...
		// check params:
		if (!$this->application->onConnecting($this, $path_params))
		{
    ...
    {		
		...
		{
        	$this->log('Send failure, closing socket ' . $payload, 'info');
			$this->server->removeClientOnError($this);
			return false;
		}
		return true;
	...
	{
		if ($value == "@no_val@") {
			if (!is_array($this->_session) || !array_key_exists($name, $this->_session))
				return null;
